...
How it works: a JavaScript Proxy intercepts registerTool on your McpServer instance. For each tool call, the middleware classifies risk, checks oversight requirements, executes the tool, and logs the result. All non-intercepted methods pass through via Reflect.get. Compliance layer errors never break tool execution.
